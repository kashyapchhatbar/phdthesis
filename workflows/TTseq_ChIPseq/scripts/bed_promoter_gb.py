...
def get_promoter_gb(row, description="pgb"):
    length = row[2] - row[1] - 1000
    if length <= 0:
        return get_promoter(row.copy(), description=f"pgb")
    if row[5] == "-":
        row[2] = row[2] - 1000        
    elif row[5] == "+":
        row[1] = row[1] - 1000        
...
def get_gene_body(row):
    length = row[2] - row[1] - 1000
    if length <= 0:
        return get_promoter(row.copy(), description=f"gb")
    oldrow = get_promoter(row.copy(), description=f"promoter")
    if row[5] == "-":                        
        end = oldrow[1]
        row[2] = end
    elif row[5] == "+":
...
def bed_gene_body(bed, size, bed_quartiles):
    ...
    quartiles.append(genes.apply(get_gene_body, axis=1))
    # quartiles.append(genes.apply(get_promoter_gb, axis=1))
...
bed_gene_body(snakemake.input[0], snakemake.input[1], snakemake.output[0])